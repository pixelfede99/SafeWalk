...
export function CircleSwitcher({ uid, deviceIds, activeDeviceId }: Props) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 text-sm bg-bg-elevated hover:bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 transition-transform duration-150 motion-safe:active:scale-[0.97]"
      >
        <span className="w-2 h-2 rounded-full bg-accent" />
        <span className="font-medium">{active?.name ?? "Círculo"}</span>
        <svg
          viewBox="0 0 24 24"
          className={`w-3 h-3 transition-transform duration-200 motion-reduce:transition-none ${open ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          strokeWidth={2}
        >
      ...
      {open && (
        <>
          ...
          />
          <div className="absolute right-0 top-full mt-2 w-64 bg-bg-card border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-popover-in origin-top-right">
